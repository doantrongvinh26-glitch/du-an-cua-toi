...
# -*- coding: utf-8 -*-
r"""
flow_automation.py — Gửi prompt hàng loạt lên Google Flow (https://flow.google.com)
bằng Playwright, trên phiên trình duyệt mà BẠN ĐÃ TỰ ĐĂNG NHẬP SẴN.

Script này KHÔNG đăng nhập hộ, KHÔNG đọc/lưu mật khẩu, KHÔNG vượt qua bất kỳ bước
xác thực nào. Nó gắn vào một cửa sổ Chrome đang mở (hoặc mở một profile có sẵn trên
máy bạn), rồi thao tác đúng như khi bạn ngồi bấm: điền prompt, chọn thiết lập đầu ra,
bấm nút tạo, theo dõi tiến độ, ghi lại kết quả.
...
----------------------
    ...
    # trên profile mặc định), rồi tự tay đăng nhập Google + mở Flow trong cửa sổ đó:
...
---------
    ...
    # Nhiều prompt lấy từ file (mỗi dòng một prompt)
    python3 flow/flow_automation.py --project abc123 --prompts-file canh.txt \
        --resolution 720p --duration 8

    # Thử trước mà không thật sự bấm nút tạo (không tốn credit)
    python3 flow/flow_automation.py --project abc123 --prompt "thử" --dry-run

Kết quả mỗi lần chạy được ghi thêm vào `jobs_history.json` (mã tác vụ, prompt,
thời điểm, trạng thái, link media nếu bắt được).

Lưu ý: Flow không có tài liệu API công khai. Mọi thứ phụ thuộc giao diện/endpoint
của Google đều gom trong phần "PHẦN PHỤ THUỘC GOOGLE FLOW" ngay dưới đây — giao diện
...
from typing import (
    TYPE_CHECKING, Any, Callable, Dict, Iterable, List, Optional, Sequence, Tuple,
)

if TYPE_CHECKING:  # chỉ phục vụ type hint — xem --help không cần cài playwright
    from playwright.sync_api import (
        Browser, BrowserContext, Locator, Page, Playwright, Request, Response,
    )

__all__ = [
    "FlowConfig", "FlowAutomation", "FlowPage", "NetworkMonitor",
    "JobRecord", "JobHistory", "SessionState", "FlowError",
]
...
class JobHistory:
    ...
    def load(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            LOG.warning("File lịch sử %s không đọc được (%s); giữ lại ở %s", self.path, exc, backup)
            try:
...
class FlowPage:
    """
    Bọc các thao tác trên một tab Flow: mở project, tìm ô prompt, chọn thiết lập, bấm tạo.

    Nguyên tắc chống vỡ khi Google đổi giao diện: mỗi thành phần đều có NHIỀU cách
    tìm, xếp từ bền nhất (vai trò + nhãn hiển thị) đến tạm bợ nhất (thẻ HTML thô).
    Thử lần lượt, cái nào hiện ra trước thì dùng. Tuyệt đối không dùng XPath cứng.
    """
    ...
    def _first_visible(
        self, what: str, builders: Sequence[LocatorBuilder], timeout_ms: int
    ) -> "Locator":
        """
        Trả về locator đầu tiên nhìn thấy được trong danh sách cách tìm.

        Quay vòng lại từ đầu cho tới khi hết giờ, vì giao diện Flow dựng dần —
        cách tìm bền nhất có thể chưa sẵn sàng ở vòng đầu.
        """
        ...
        while True:
            for name, build in builders:
                ...
                try:
                    locator = build().first
                    locator.wait_for(
                        state="visible", timeout=max(250, min(1500, remaining * 1000))
                    )
    ...
    def prompt_input(self, timeout_ms: Optional[int] = None) -> "Locator":
        """Ô nhập prompt chính ("What do you want to create?")."""
        page = self.page
        ...
        return self._first_visible(
            "ô nhập prompt", builders, timeout_ms or self.config.ready_timeout_ms
        )

    def submit_button(self, timeout_ms: int = 4000) -> Optional["Locator"]:
        """Nút gửi (mũi tên / Create). Không thấy thì trả None để chuyển sang nhấn Enter."""
        page = self.page
        builders: List[LocatorBuilder] = [
        ...
        try:
            return self._first_visible("nút gửi", builders, timeout_ms)
    ...
    def ensure_project(self) -> str:
        """
        Đưa tab về đúng URL project và chờ giao diện sẵn sàng.

        Không truyền --project thì chấp nhận tab đang mở, miễn là đang ở Flow.
        """
        target = resolve_project_url(self.config.project, self.config.base_url)
        ...
        elif self.config.host not in current:
            raise NavigationFailed(
                "Tab đang mở không phải Google Flow và bạn chưa truyền --project. "
                "Hãy mở sẵn project trong trình duyệt, hoặc chạy lại với "
                "--project <id-hoặc-URL>.",
                url=current,
            )
        else:
        ...
        if self.config.host not in final_url:
    ...
    def wait_until_ready(self) -> None:
        ...
        try:
            self.page.wait_for_load_state("domcontentloaded", timeout=self.config.ready_timeout_ms)
        except Exception as exc:
        ...
        for selector in ("main", "[role='main']", "canvas", "[class*='canvas']"):
            try:
                self.page.locator(selector).first.wait_for(state="attached", timeout=3000)
                LOG.debug("Khung chính đã có (%s).", selector)
                break
            except Exception:
                continue
        self.prompt_input()  # thứ này thì bắt buộc phải có
    ...
    def open_settings_panel(self) -> bool:
        ...
        builders: List[LocatorBuilder] = [
            ...
            ("icon tune", lambda: page.locator("button:has-text('tune'), button:has-text('settings')")),
        ]
        try:
            button = self._first_visible("nút thiết lập", builders, 4000)
    ...
    def apply_settings(self, wanted: Dict[str, str]) -> Dict[str, str]:
        """
        Chọn các thiết lập đầu ra (mô hình, độ phân giải, thời lượng, số bản...).

        Đây là bước "có thì tốt": Flow đổi bảng thiết lập khá thường xuyên, nên
        hỏng bước này chỉ cảnh báo chứ KHÔNG huỷ cả lượt chạy — bạn vẫn tạo được
        video, chỉ là theo thiết lập đang có sẵn trên giao diện.
        """
        if not wanted:
            return {}
        LOG.info("==> Đặt thiết lập đầu ra: %s", ", ".join(f"{k}={v}" for k, v in wanted.items()))
        if not self.open_settings_panel():
            LOG.warning(
        ...
        for name, value in wanted.items():
            try:
                if self._set_one_setting(name, value):
                    applied[name] = value
    ...
    def _set_one_setting(self, name: str, value: str) -> bool:
        ...
        # 1) Ô chọn kiểu combobox/dropdown có nhãn tương ứng.
        ...
        try:
            control = self._first_visible(f"ô chọn {name}", control_builders, 2500)
        except ElementNotFound:
            # 2) Không có ô chọn: có thể giá trị hiện thẳng thành nút/chip (ví dụ "720p").
            return self._click_value_directly(value_re)

        # Thử <select> thật trước — nhanh và chắc chắn nhất.
        try:
            if (control.evaluate("el => el.tagName") or "").upper() == "SELECT":
                control.select_option(label=str(value), timeout=self.config.action_timeout_ms)
                return True
        except Exception:
            pass

        control.click(timeout=self.config.action_timeout_ms)
        ...
        try:
            option = self._first_visible(f"giá trị {value}", option_builders, 2500)
            option.click(timeout=self.config.action_timeout_ms)
            page.wait_for_timeout(300)
            return True
        except ElementNotFound:
            # Ô nhập số (thời lượng chẳng hạn) thay vì danh sách chọn.
            try:
                control.fill(str(value), timeout=self.config.action_timeout_ms)
                return True
            except Exception:
                page.keyboard.press("Escape")
                return False

    def _click_value_directly(self, value_re: "re.Pattern[str]") -> bool:
        """Bấm thẳng vào chip/nút mang đúng giá trị (ví dụ nút "720p")."""
        for build in (
            lambda: self.page.get_by_role("button", name=value_re),
            lambda: self.page.get_by_role("radio", name=value_re),
            lambda: self.page.get_by_text(value_re),
        ):
            try:
                target = build().first
                target.wait_for(state="visible", timeout=1200)
                target.click(timeout=self.config.action_timeout_ms)
                return True
            except Exception:
                continue
        return False
    ...
    def _close_settings_panel(self) -> None:
        """Đóng bảng thiết lập để nó không che ô nhập prompt."""
        try:
    ...
    def fill_prompt(self, prompt: str) -> "Locator":
        """Điền prompt vào ô nhập và kiểm tra lại là chữ đã vào thật."""
        field_locator = self.prompt_input()
        field_locator.click(timeout=self.config.action_timeout_ms)
        try:
            field_locator.fill(prompt, timeout=self.config.action_timeout_ms)
        except Exception as exc:
        ...
        if not self._prompt_matches(field_locator, prompt):
            raise SubmitFailed(
    ...
    def _prompt_matches(self, field_locator: "Locator", prompt: str) -> bool:
    ...
    def submit(self, field_locator: "Locator") -> str:
        """Kích hoạt lệnh tạo. Trả về cách đã dùng ("nút" hay "Enter")."""
        ...
        try:
            field_locator.press("Enter", timeout=self.config.action_timeout_ms)
            return "phím Enter"
        except Exception as exc:
            raise SubmitFailed(f"Không kích hoạt được lệnh tạo: {exc}") from exc

    # --- tín hiệu từ giao diện -------------------------------------------

    def media_count(self) -> int:
        """Đếm số media đang hiện trên khung vẽ — dùng làm tín hiệu phụ báo đã xong."""
        try:
            return self.page.locator("video, video source[src]").count()
        except Exception:
            return 0
    ...
    def looks_busy(self) -> bool:
        ...
        for build in (
            lambda: self.page.get_by_text(BUSY_TEXT_RE),
            lambda: self.page.locator("[role='progressbar'], progress"),
        ):
            try:
...
# ===========================================================================
# Điều phối: nối trình duyệt, chạy cả loạt prompt, theo dõi tới khi xong
# ===========================================================================


class FlowAutomation:
    """
    Điều phối toàn bộ lượt chạy: gắn trình duyệt → mở project → gửi từng prompt → ghi lịch sử.

    Dùng như context manager để chắc chắn nhả kết nối:

        with FlowAutomation(config) as bot:
    ...
    def __init__(self, config: FlowConfig) -> None:
        ...
        self.history = JobHistory(config.history_path)
        ...
        self._page: Optional["Page"] = None
        self._flow: Optional[FlowPage] = None
        # Tab do script tự mở thì mới được phép đóng lúc kết thúc.
        self._owns_page = False
        # Trình duyệt do script tự khởi động thì mới được phép tắt lúc kết thúc.
        self._owns_browser = False

    # --- vòng đời ---------------------------------------------------------

    def __enter__(self) -> "FlowAutomation":
        self.connect()
        return self
    ...
    def connect(self) -> None:
        ...
        Cả hai lối đều dựa trên phiên bạn đã tự đăng nhập — script không tự đăng nhập.
        """
        try:
        ...
        self._flow = FlowPage(self._page, self.config)
        self.monitor.attach(self._page)

    def _connect_over_cdp(self) -> None:
        """Gắn vào cửa sổ Chrome đang mở sẵn cổng gỡ lỗi."""
        assert self._playwright is not None
        LOG.info("==> Gắn vào Chrome qua CDP: %s", self.config.cdp_url)
        try:
            self._browser = self._playwright.chromium.connect_over_cdp(self.config.cdp_url)
        except Exception as exc:
            raise ConnectionFailed(
                f"Không gắn được vào {self.config.cdp_url}: {exc}\n"
        ...
        # Trình duyệt là của người dùng: kết thúc chỉ ngắt kết nối, không tắt.
        self._owns_browser = False
    ...
    def _launch_persistent(self) -> None:
        ...
        try:
            self._context = self._playwright.chromium.launch_persistent_context(
                str(user_data_dir),
                channel=self.config.channel or None,
                executable_path=(
                    str(self.config.executable_path) if self.config.executable_path else None
                ),
                headless=self.config.headless,
                args=["--disable-blink-features=AutomationControlled", *self.config.browser_args],
            )
        except Exception as exc:
            raise ConnectionFailed(
    ...
    def _pick_page(self) -> "Page":
        """Chọn tab đang ở Flow; không có thì mở tab mới."""
        assert self._context is not None
        for page in self._context.pages:
            try:
                if self.config.host in (page.url or ""):
                    LOG.info("    Dùng tab Flow đang mở: %s", page.url)
                    page.bring_to_front()
                    return page
            except Exception:
                continue
        if self._context.pages and not self.config.project:
            # Không biết mở project nào thì cứ dùng tab hiện có và để ensure_project báo lỗi rõ.
    ...
    def close(self) -> None:
        """Nhả mọi thứ, tuyệt đối không tắt trình duyệt của người dùng."""
        self.monitor.detach()
        if self._owns_page and self.config.close_tab and self._page is not None:
            try:
                self._page.close()
            except Exception as exc:  # pragma: no cover
    ...
    def page(self) -> "Page":
        if self._page is None:
            raise FlowError("Chưa kết nối trình duyệt — gọi connect() trước.")
        return self._page

    @property
    def flow(self) -> FlowPage:
    ...
    def _sleep(self, seconds: float) -> None:
        ...
        """
        try:
            self.page.wait_for_timeout(max(0, seconds) * 1000)
        except Exception:  # pragma: no cover - tab có thể đã đóng
            time.sleep(max(0, seconds))

    def capture_session(self) -> SessionState:
        """Chụp lại cookie/header của phiên đang đăng nhập."""
        assert self._context is not None
        self.session = SessionState.capture(
            self._context, self.monitor.observed_headers, self.config.base_url
        )
        LOG.info("==> Phiên đăng nhập: %s", json.dumps(self.session.summary(), ensure_ascii=False))
        if self.config.session_out:
            self.session.save(self.config.session_out)
        return self.session

    def verify_media(self, url: str) -> Optional[int]:
        """
        Kiểm tra link media có mở được bằng chính phiên của người dùng không.

        Dùng APIRequestContext của trình duyệt nên cookie đi kèm sẵn — không phải
        sao chép thông tin đăng nhập ra ngoài.
        """
        if self._context is None:
            return None
        try:
            response = self._context.request.head(url, timeout=15_000)
    ...
    def run_batch(self, prompts: Sequence[str]) -> List[JobRecord]:
        """Gửi lần lượt từng prompt, ghi lịch sử sau MỖI prompt (ngắt giữa chừng không mất dữ liệu)."""
        project_url = self.flow.ensure_project()
        project_id = project_id_from_url(project_url)
        LOG.info("==> Project: %s", project_id or "(không rõ id)")

        requested = self.config.desired_settings()
        applied: Dict[str, str] = {}
        if self.config.apply_settings:
            applied = self.flow.apply_settings(requested)
        elif requested:
            LOG.warning("Có --no-settings nên bỏ qua: %s", ", ".join(requested))
        ...
        for index, prompt in enumerate(prompts, start=1):
            ...
            record = JobRecord(
                prompt=prompt, index=index, project_id=project_id,
                project_url=project_url, settings=requested, settings_applied=applied,
            )
            try:
                self.run_one(record)
                ...
                record.finish("error", error=f"{type(exc).__name__}: {exc}")

            records.append(record)
            self.history.append([record])  # ghi ngay, không đợi hết loạt
            LOG.info("    Kết quả: %s%s", record.status,
                     f" ({record.duration_s}s)" if record.duration_s else "")

            if index < total and self.config.delay_between_s > 0:
                LOG.debug("Nghỉ %.1fs trước prompt kế tiếp.", self.config.delay_between_s)
                self._sleep(self.config.delay_between_s)

        return records

    def run_one(self, record: JobRecord) -> JobRecord:
        """Gửi một prompt rồi theo dõi tới khi xong / lỗi / hết giờ."""
        baseline_media = self.flow.media_count()
        mark = self.monitor.mark()

        field_locator = self.flow.fill_prompt(record.prompt)

        if self.config.dry_run:
            LOG.info("    [dry-run] Đã điền prompt, KHÔNG bấm nút tạo.")
            record.note("dry-run: chỉ điền prompt")
            record.finish("dry-run")
            return record

        how = self.flow.submit(field_locator)
        record.submitted_at = now_iso()
        record.status = "submitted"
        record.note(f"đã gửi bằng {how}")
        LOG.info("    Đã gửi (%s). Đang chờ kết quả...", how)

        self.wait_for_generation(record, mark, baseline_media)
        return record

    def wait_for_generation(self, record: JobRecord, mark: int, baseline_media: int) -> None:
        """
        ...
        delays = backoff_delays(
            self.config.poll_initial_s, self.config.poll_factor, self.config.poll_max_s
        )
        cursor = mark
        last_log = 0.0
        ...
        while True:
            # --- 1. tín hiệu mạng ---
            events = self.monitor.events[cursor:]
            cursor = len(self.monitor.events)
            for event in events:
                self._absorb_event(record, event)
                if record.status in ("done", "failed"):
                    self._finalize(record)
                    return

            # --- 2. tín hiệu giao diện ---
            current_media = self.flow.media_count()
            if current_media > baseline_media and not self.flow.looks_busy():
                record.note(
                    f"giao diện: số media tăng {baseline_media} → {current_media}"
                )
                record.finish("done")
                self._finalize(record)
                return
            ...
            if remaining <= 0:
                record.finish(
                    "timeout",
                    error=(
                        f"Quá {self.config.generation_timeout_s:.0f}s mà chưa thấy báo xong. "
                        "Tác vụ có thể vẫn đang chạy — mở lại project để xem, "
                        "hoặc tăng --timeout."
                    ),
                )
                self._finalize(record)
                raise GenerationTimeout(str(record.error), task_id=record.task_id)
            ...
            if elapsed - last_log >= 15:  # báo tiến độ 15s một lần cho đỡ ồn
                LOG.info("    ... đang chạy %.0fs (mã tác vụ: %s)",
                         elapsed, record.task_id or "chưa bắt được")
                last_log = elapsed
            self._sleep(delay)

    def _absorb_event(self, record: JobRecord, event: NetEvent) -> None:
        """Cập nhật bản ghi theo một sự kiện mạng, nếu sự kiện đó thuộc về tác vụ này."""
        if event.kind == "generate" and event.task_ids and not record.task_ids:
            record.task_ids = list(event.task_ids)
            record.task_id = event.task_ids[0]
            record.note("mã tác vụ lấy từ response tạo video")
            LOG.info("    Mã tác vụ: %s", record.task_id)
        ...
        if event.kind == "generate" and event.status_code >= 400:
            record.finish("failed", error=event.error or f"Flow trả về HTTP {event.status_code}")
            record.note(f"mạng: HTTP {event.status_code} khi gửi tác vụ")
            return

        if not self._event_matches(record, event):
            return

        for url in event.media_urls:
            if url not in record.media_urls:
                record.media_urls.append(url)

        if event.state == "failed":
            record.finish("failed", error=event.error or "Flow báo tác vụ thất bại.")
            record.note("mạng: trạng thái thất bại")
        elif event.state == "done":
            record.finish("done")
            record.note("mạng: trạng thái hoàn tất")

    @staticmethod
    def _event_matches(record: JobRecord, event: NetEvent) -> bool:
        """
        Sự kiện này có nói về tác vụ đang theo dõi không?

        Khớp theo mã tác vụ khi đã biết mã. Chưa biết mã thì chấp nhận sự kiện
        trạng thái bất kỳ — vì các prompt chạy tuần tự nên tại một thời điểm chỉ
        có đúng một tác vụ do script này gửi đang chờ.
        """
        if record.task_ids and event.task_ids:
            return bool(set(record.task_ids) & set(event.task_ids))
    ...
    def _finalize(self, record: JobRecord) -> None:
        ...
        if record.status == "done" and record.media_urls:
            LOG.info("    Bắt được %d link media.", len(record.media_urls))
            if self.config.verify_media:
                status = self.verify_media(record.media_urls[0])
                record.note(f"kiểm tra link media: HTTP {status}")
        if record.status == "failed":
            LOG.warning("    Tác vụ thất bại: %s", record.error)
